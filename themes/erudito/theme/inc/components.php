...
?>
    ...
    <div class="relative <?php echo esc_attr($config['class']); ?>"
        id="<?php echo esc_attr($config['id']); ?>"
        x-data="erdDropdown(<?php echo esc_attr(json_encode($alpine_data)); ?>)"
        ...
        <button type="button"
            ...
            :class="open ? 'border-gray3' : 'border-gray'"
            @click="open = !open"
            :aria-expanded="open"
            ...
            <svg class="absolute w-6 h-6 text-black transition-transform duration-200 transform -translate-y-1/2 right-4 top-1/2"
                ...
                <path d="M18 10.5L12.0005 14.25L6 10.5" stroke="currentColor" stroke-width="1.5" stroke-miterlimit="10" stroke-linecap="square" />
            </svg>
        </button>
        ...
        <div class="absolute z-10 w-full mt-2 overflow-hidden transition-all duration-200 bg-white border rounded-lg erd-dropdown-menu border-gray3"
            ...
            <?php foreach ($config['options'] as $value => $label): ?>
                <a href="#"
                    class="flex items-center justify-between px-4 py-3 transition-color hover:bg-gray text-body-m-light"
